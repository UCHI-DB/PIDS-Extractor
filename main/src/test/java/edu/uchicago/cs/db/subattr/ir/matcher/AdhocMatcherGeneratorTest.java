...
public class AdhocMatcherGeneratorTest {
    ...
    public void testGenerate() {
        ...
        ExactMatcher matcher = new AdhocMatcherGenerator().generate(pattern);

        CharSequence[] matched = matcher.match("234-4111");
        assertFalse(matched == null);
        assertEquals(2, matched.length);
        assertEquals("234", matched[0]);
        assertEquals("4111", matched[1]);
    ...
    public void testAddress() {
        ...
        assertEquals("460", matched[0]);
        assertEquals("Maple Spruce", matched[1]);
        assertEquals("Court", matched[2]);
        assertEquals("480", matched[3]);
        assertEquals("Somerville", matched[4]);
        assertEquals("Potter County", matched[5]);
        assertEquals("SD", matched[6]);
        assertEquals("57783", matched[7]);
        String operand2 = "|Sunset Pine||Suite ||Dickson County|TN|";

        matched = matcher.match(operand2);
        assertEquals(8, matched.length);
        assertEquals("", matched[0]);
        assertEquals("Sunset Pine", matched[1]);
        assertEquals("", matched[2]);
        assertEquals("", matched[3]);
        assertEquals("", matched[4]);
        assertEquals("Dickson County", matched[5]);
        assertEquals("TN", matched[6]);
        assertEquals("", matched[7]);
    ...
    public void testVariousAny() {
        ...
        assertEquals("ABBA",matched[1]);
        assertEquals("322",matched[2]);

        matched = matcher.match("323|Ab|55A");
        assertEquals(3, matched.length);
        assertEquals("323",matched[0]);
        assertEquals("Ab",matched[1]);
        assertEquals("55A",matched[2]);
