...
import edu.uchicago.cs.db.wordvec.{DbWordSource, FileWordSource}
...
class FrequentWord {
  ...
  private val dict = new WordEmbedDict(new FileWordSource(new File(Thread.currentThread().getContextClassLoader
    .getResource("glove_dict.txt").toURI).getAbsolutePath))
