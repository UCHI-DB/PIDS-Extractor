...
import edu.uchicago.cs.db.wordvec.SimilarWord

import scala.collection.mutable.ArrayBuffer

object CommonSeqEqualFunc {

  def exactEquals(a: Pattern, b: Pattern): Boolean = {
    a.getClass == b.getClass && a.numChar == b.numChar
  }
    ...
    seq.isEmpty match {
      ...
      case false => {
        ...
        val fuzzySymbols = exactMatch match {
          case true => Array()
          case false => Array.fill(seq.length)(new ArrayBuffer[Pattern]())
        }
        ...
        commonPos.indices.foreach(j => {
          ...
          pos.indices.foreach(i => {
            val sec = pos(i)
            sectionBuffers(i) += PSeq(data.view(pointer, sec))
            pointer = sec + 1

            if (!exactMatch)
              fuzzySymbols(i) += data(sec)
          })
        ...
        val symbols = exactMatch match {
          case true => seq
          case false => fuzzySymbols.map(PUnion(_)).toSeq
        }
        // Create new pattern
