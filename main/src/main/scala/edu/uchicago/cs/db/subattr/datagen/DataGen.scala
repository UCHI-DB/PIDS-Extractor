...
object DataGen extends App {
  ...
  def randRange(upper: Int): String = {
    Random.nextInt(upper).formatted("%0" + upper + "d");
  }
  ...
  def genDate(copy: Int, process: String => Unit) = {
  ...
  def genTime(copy: Int, process: String => Unit) = {
    // hh:mm:ss
  ...
  def genPhone1(copy: Int, process: String => Unit): Unit = {
    ...
    while (i < copy) {
      ...
      process("(%s)%s-%s".format(a, b, c))
      i += 1
    }
  }

  def genIpAddress(copy: Int, process: String => Unit) = {
  ...
  def genPart: Unit = {
    ...
    genDate(5000, writer.println)
    genTime(5000, writer.println)
    genIpAddress(5000, writer.println)
    genPhone1(5000, writer.println)
    writer.close()
  }
  ...
  val fileWriter = new PrintWriter(new FileOutputStream("ipv6.raw"))
  genIpAddress(150000, fileWriter.println)
  fileWriter.close()
}
